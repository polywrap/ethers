...
pub fn from_wrap_tx_options(maybe_options: Option<TxOptions>) -> EthersTxOptions {
    match maybe_options {
        Some(options) => EthersTxOptions {
            gas_limit: options.gas_limit.map(|big_int| bigint_to_u256(&big_int.0)),
            max_fee_per_gas: options.max_fee_per_gas.map(|big_int| bigint_to_u256(&big_int.0)),
            max_priority_fee_per_gas: options.max_priority_fee_per_gas.map(|big_int| bigint_to_u256(&big_int.0)),
            gas_price: options.gas_price.map(|big_int| bigint_to_u256(&big_int.0)),
            value: options.value.map(|big_int| bigint_to_u256(&big_int.0)),
...
pub fn to_wrap_response(
    ...
    TxResponse {
        ...
        gas_limit: BigInt::from_str(&response.gas.to_string())
            .map(|b| BigIntWrapper(b))
            .unwrap(),
        max_fee_per_gas,
        max_priority_fee_per_gas,
        ...
        block_number: response.block_number
            .map(|n| n.to_string())
            .map(|n| BigInt::from_str(&n).unwrap())
            .map(|b| BigIntWrapper(b)),
