...
impl ModuleTrait for Module {
    ...
    fn get_balance(args: wrap::ArgsGetBalance) -> Result<BigIntWrapper, String> {
        ...
        BigInt::from_str(&balance.unwrap().to_string())
            .map_err(|e| e.to_string())
            .map(|gas| BigIntWrapper(gas))
    }
    ...
    fn estimate_eip1559_fees(args: wrap::ArgsEstimateEip1559Fees) -> Result<wrap::Eip1559FeesEstimate, String> {
        ...
        let max_fee_per_gas = BigInt::from_str(&price.0.to_string()).unwrap();
        let max_priority_fee_per_gas = BigInt::from_str(&price.1.to_string()).unwrap();
        Ok(wrap::Eip1559FeesEstimate {
            max_fee_per_gas: BigIntWrapper(max_fee_per_gas),
            max_priority_fee_per_gas: BigIntWrapper(max_priority_fee_per_gas),
        })
    }
